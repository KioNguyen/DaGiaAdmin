...
class AddForm extends React.Component {
    ...
    categoryTree(category) {
        let categoryTree = category.reduce((arr, ele) => {
            let obj = {
                title: ele.name,
                value: ele._id
            }
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.editModalVisible !== editModalVisible) {
            this.setState({ editModalVisible: nextProps.editModalVisible })
        }
    }
    ...
    getTime(type) {
        let timeStart = this.props.productEdit.auction ? Date.parse(this.props.productEdit.auction.timeDuration) : 0
        const days = Math.floor(timeStart / (1000 * 60 * 60 * 24));
        const hours = Math.floor((timeStart % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
        const minutes = Math.floor((timeStart % (1000 * 60 * 60)) / (1000 * 60));
        const seconds = Math.floor((timeStart % (1000 * 60)) / 1000);
        console.log(Date.parse(timeStart))
        switch (type) {
            case "DAY":
                return days
                break;
    ...
    handleCancelModal = e => {
        ...
        this.props.callbackVisibleEditForm(false)
    };
    handleOpenModal = e => {
        console.log(e);
        this.setState({
            ...
            >
                ...
                >
                    <Form.Item label="Tên sản phẩm"
                        ...
                        <Input
                            defaultValue={this.props.productEdit.name}
                    ...
                    <Form.Item label="Tình trạng">
                        <InputNumber
                            defaultValue={this.props.productEdit.condition}
                            min={0}
                            max={100}
                    ...
                    <Form.Item label="Danh mục"
                        ...
                        <TreeSelect
                            inputValue={"5eabb5e7b9a6656704b38b03"}
                            placeholder="Chọn danh mục sản phẩm"
                            treeData={this.categoryTree(this.props.category)}
                    ...
                    <Form.Item label="Giá khởi điểm"
                        ...
                        <InputNumber
                            defaultValue={this.props.productEdit.auction ? this.props.productEdit.auction.startPrice : 0}
                    ...
                    <Form.Item label="Bước giá"
                        ...
                        <InputNumber
                            ...
                            defaultValue={this.props.productEdit.auction ? this.props.productEdit.auction.priceSte : 0}
                            formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                            parser={value => value.replace(/\$\s?|(,*)/g, '')}
                            step={1000}
                    ...
                    <Form.Item label="Thời gian đấu giá"
                        ...
                        <InputNumber
                            className='InputTimeStart'
                            defaultValue={this.getTime('HOUR')}
                            min={0}
                            max={23}
                        ...
                        <InputNumber
                            className='InputTimeStart'
                            defaultValue={this.getTime('MINU')}
                            min={0}
                            max={59}
                        ...
                        <InputNumber
                            className='InputTimeStart'
                            defaultValue={this.getTime('SECO')}
                            min={0}
                            max={59}
